...
APP_TITLE = "GT_SUPER KONTROLER 2026 V001F STABLE"
...
MOUSE_LABELS = {
    ...
    "ENABLE_GP": "GP19 DIP Aktif", "SWLEVEL": "GP19 Açık Seviyesi (0/1)", "CAL_GP": "Kalibrasyon GP", "EDGE_L": "Sol Kenar", "EDGE_R": "Sağ Kenar",
...
DEFAULT_MOUSE = {
    ...
    "ENABLE_GP": 19, "SWLEVEL": 1, "CAL_GP": 255,
...
class Device:
    ...
    def parse(self, line):
        ...
        if line.startswith("RAWNOW,"):
            parts = line.split(",")
            try:
                i = parts.index("RAW")
                self.raw_now = (int(parts[i+1]), int(parts[i+2]))
                self.raw_seq += 1
            except Exception:
                pass
        self.q.put(line)
...
class App(tk.Tk):
    def __init__(self):
        ...
        if os.name != "nt":
            self.bind("<F5>", self.toggle_mouse_system)
        ...
        self._last_t1 = 0
        self._last_t2 = 0
        self._last_cal_confirm_ms = {"P1": 0, "P2": 0}
        self._f5_down = False
        ...
        self.create_ui()
        self.after(100, self.poll_global_f5)
    ...
    def poll_global_f5(self):
        """Windows'ta program odakta olmasa da F5'i yakalar."""
        if os.name == "nt":
            try:
                down = bool(ctypes.windll.user32.GetAsyncKeyState(0x74) & 0x8000)
            except Exception:
                down = False
            if down and not self._f5_down:
                self.toggle_mouse_system()
            self._f5_down = down
        self.after(50, self.poll_global_f5)

    def toggle_mouse_system(self, event=None):
        sent = []
        for player in ("P1", "P2"):
            ...
            if d:
                ...
                sent.append(player)
        if sent:
            self.status.config(text=f"F5: {' ve '.join(sent)} mouse sistemi sanal olarak aç/kapat değiştirildi.")
        else:
            self.status.config(text="F5 basıldı fakat P1/P2 Pico bulunamadı. Önce Cihazları Tara.")
        return "break"
    ...
    def create_ui(self):
        ...
        self.status = ttk.Label(self, text="GT_SUPER KONTROLER V001F hazır. GP2-GP11 = klavye 1-0. F5, P1/P2 GP19 anahtarını sanal aç/kapatır. ESC tam ekrandan çıkar.", padding=(12,10), relief="groove", font=("Segoe UI", 11, "bold"), background="#fef3c7", foreground="#7c2d12")
    ...
    def build_ctrl_tab(self):
        ...
        ttk.Label(f, text="Standart: GP2=1 Coin, GP3=2 P1 Start, GP4=3 P1 Tetik, GP5=4 P1 Bomba, GP6=5 P2 Start, GP7=6 P2 Tetik, GP8=7 P2 Bomba, GP9=8, GP10=9, GP11=0.").grid(row=1, column=0, columnspan=6, sticky="w", pady=(0,4))
    ...
    def build_mouse_tab(self, player, parent):
        ...
        swbox = ttk.LabelFrame(f, text="GP19 Anahtar Ayarı", padding=8); swbox.pack(fill="x", pady=6)
        ttk.Label(swbox, text="Anahtarı fiziksel KAPALI konuma getirip bu düğmeye bir kez bas.").pack(side="left", padx=4)
        ttk.Button(swbox, text="GP19 KAPALI Konumu Öğret", command=lambda p=player: self.learn_gp19_off(p), style="Accent.TButton").pack(side="left", padx=6)
...
11) GP2..GP11 sırasıyla klavye 1,2,3,4,5,6,7,8,9,0 olarak gerçek basılı/bırakılı çalışır.
...
18) F5, P1/P2 GP19 anahtarlarını yazılımsal olarak tersler; fiziksel anahtar açıkken kapatabilir, kapalıyken açabilir.
19) GP19 yönü tersse anahtarı KAPALI konuma getirip “GP19 KAPALI Konumu Öğret” düğmesine bas.
"""
    ...
    def learn_gp19_off(self, player):
        d = self.get_dev(player)
        if not d:
            messagebox.showwarning("Cihaz yok", f"{player} Pico bulunamadı. Önce Cihazları Tara.")
            return
        d.send("STATUS")
        time.sleep(0.12)
        raw = self._status_value(d.last_status, "GP19RAW", self._status_value(d.last_status, "DIP19", -1))
        if raw not in (0, 1):
            messagebox.showwarning("GP19 okunamadı", "Yeni P1/P2 UF2'yi yüklediğinden emin ol.")
            return
        on_level = 0 if raw == 1 else 1
        self.mouse_vars[player]["SWLEVEL"].set(str(on_level))
        d.send(f"SET,SWLEVEL,{on_level}")
        time.sleep(0.05)
        d.send("SAVE")
        time.sleep(0.08)
        d.send("GETCFG")
        self.save_local(f"{player} GP19 kapalı konumu öğretildi")
        messagebox.showinfo("GP19 Kaydedildi", f"{player}: kapalı seviye={raw}, açık seviye={on_level} kaydedildi.")
    ...
    def start_calibration_flow(self, player):
        ...
        self.cal_start_ms[player] = int(time.time() * 1000)
    ...
    def capture_corner(self, player, code, method="Mouse", show_warning=True):
        ...
        if not d:
            if show_warning:
                messagebox.showwarning("Veri yok", f"{player} bulunamadı. Cihazları Tara yap.")
            return False
        samples = []
        for _ in range(3):
            before = d.raw_seq
            ...
            while time.time() < deadline and d.raw_seq == before:
                time.sleep(0.005)
            if d.raw_seq != before and d.raw_now:
                samples.append(d.raw_now)
            time.sleep(0.015)
        if not samples and d.last_status:
            parts = d.last_status.split(",")
            try:
                i = parts.index("RAW")
                samples.append((int(parts[i+1]), int(parts[i+2])))
            except Exception:
                pass
        if not samples:
            if show_warning:
                messagebox.showwarning("RAW okunamadı", f"{player} anlık potans değerini göndermedi. Yeni UF2'yi yükle.")
            return False
        x = int(statistics.median(v[0] for v in samples))
        y = int(statistics.median(v[1] for v in samples))
        self.capture[player][code] = (x, y)
        step_name = dict(CAL_STEPS).get(code, code)
        log = getattr(self, f"{player}_log", None)
        if log:
            log.insert("end", f"{step_name} [{method}] = RAW {x},{y} ({len(samples)} örnek)\n")
            log.see("end")
        return True
    ...
    def write_mouse_cal(self, player):
        ...
        normal_score = (abs(h0) / (abs(v0) + 1.0)) + (abs(v1) / (abs(h1) + 1.0))
        swap_score = (abs(h1) / (abs(v1) + 1.0)) + (abs(v0) / (abs(h0) + 1.0))
        ...
        channels = [current_x_adc, current_y_adc]
        new_x_adc, new_y_adc = channels[x_index], channels[y_index]
        ...
        left_mean = (cap["LU"][x_index] + cap["LD"][x_index]) / 2.0
        right_mean = (cap["RU"][x_index] + cap["RD"][x_index]) / 2.0
        top_mean = (cap["LU"][y_index] + cap["RU"][y_index]) / 2.0
        bottom_mean = (cap["LD"][y_index] + cap["RD"][y_index]) / 2.0
        invx = 1 if right_mean < left_mean else 0
        invy = 1 if bottom_mean < top_mean else 0
        x_range, y_range = xmax - xmin, ymax - ymin
        if x_range < 80 or y_range < 80:
            messagebox.showwarning("Kalibrasyon hareketi yetersiz", f"X aralığı={x_range}, Y aralığı={y_range}. Köşeleri daha belirgin göster.")
            return
        for key, value in (("XMIN", xmin), ("XMAX", xmax), ("YMIN", ymin), ("YMAX", ymax), ("INVX", invx), ("INVY", invy)):
            self.mouse_vars[player][key].set(str(value))
        log = getattr(self, f"{player}_log", None)
        if log:
            durum = "X/Y değiştirildi" if swap_axes else "X/Y sırası doğru"
            log.insert("end", f"Otomatik eksen: {durum}; X=ADC{new_x_adc}, Y=ADC{new_y_adc}; X ters={invx}, Y ters={invy}\n")
            log.see("end")
        if not d:
            messagebox.showwarning("Cihaz yok", f"{player} bulunamadı; ayar yazılamadı.")
            return
        d.send(f"CALXY,{new_x_adc},{new_y_adc},{xmin},{xmax},{ymin},{ymax},{invx},{invy}")
        time.sleep(0.18)
        d.send("GETCFG")
        d.send("STATUS")
        ...
        messagebox.showinfo("Tamam", f"{player} kalibrasyonu kaydedildi.\n{sonuc}. X=ADC{new_x_adc}, Y=ADC{new_y_adc}.\nX ters={invx}, Y ters={invy}.")
    ...
    def handle_serial_event(self, device, line):
        if not line.startswith("EVENT,"):
            return
        parts = line.split(",")
        if len(parts) >= 4 and parts[1] == "TRIGGER" and parts[2] in ("P1", "P2"):
            player = parts[2]
            pressed = parts[3] == "1"
            if pressed and self.cal_active.get(player, False):
                now_ms = int(time.time() * 1000)
                if now_ms - self._last_cal_confirm_ms[player] >= 250:
                    self._last_cal_confirm_ms[player] = now_ms
                    self.confirm_calibration_step(player, "Tetik")
    ...
    def check_trigger_calibration(self):
        ...
        now_ms = int(time.time() * 1000)
        if t1 and not self._last_t1 and self.cal_active.get("P1", False):
            if now_ms - self._last_cal_confirm_ms["P1"] >= 250:
                self._last_cal_confirm_ms["P1"] = now_ms
                self.confirm_calibration_step("P1", "Tetik")
        if t2 and not self._last_t2 and self.cal_active.get("P2", False):
            if now_ms - self._last_cal_confirm_ms["P2"] >= 250:
                self._last_cal_confirm_ms["P2"] = now_ms
    ...
    def refresh(self):
        ...
        for port, d in self.devices.items():
            ...
            while not d.q.empty():
                line = d.q.get_nowait()
                self.handle_serial_event(d, line)
